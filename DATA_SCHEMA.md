...
Blockets form: `{lage, kalla, vintage, fonsterDagar, raw?, kommentar, regim:[{id, namn, mekanik, delta5d}], rader:[{ticker, accBp, netDelta?, nHus?, nRev?, nReinit?, largest?, sourceStale?, uppsida, analytiker, dagar?}]}`.
...
- `netDelta` (rådata) = **riktning**: summan av `delta_pct` för de **färska** revisionerna i samma 30d-fönster som `accBp` (reiterationer OCH reinit räknas ej), i %. `null` vid 0 färska revisioner. **Styr radens pil** — d² ensam vilseleder efter stegrörelser (brant fall → platt läses som positiv böjning), så riktningen läses först. Panelen: `▼ −8,2 % 30d · acc +0,2 bp/d² · 11 hus`. Obs: net_delta är en SUMMA, så den skalar med antalet hus (25 färska hus × ~4,5 % = +112 %) — stort tal ≠ fel när `nRev` är högt.
- `nReinit` (rådata) = revisioner mot en **flerårig prior** (härledd prior äldre än `p_reinit_days`, default **365 d**, frikopplad från carry-forwardens 180 d). Sådana är **täckningsåterinitiering**, inte riktningsändringar — de exkluderas ur `netDelta`/`nRev`/`nHus`/kluster och räknas här. Panelen visar "· N reinit"; är alla fönsterrevisioner reinit (nRev 0, nReinit > 0) visas "återinit (N)". Rättar stale-prior-föroreningen (META $140→$650-fossilen gav förr +364 %). Se `tpa/README.md`.
...
- **Snapshot-artefaktvarning** (endast snapshot-vägen): ändras `analyst_count` > ±25 % i fönstret (`tpaBasandring` ↔ `analytikerBasandring`) räknas konsensus-snittet om över en annan husmängd → snapshot-`accBp` blir ett **kompositionsskifte, inte en riktkursrörelse** (META 3→12 den 31/7 gav −5,18). Sådana rader **dämpas** + title "konsensusbas ändrad — acc opålitlig" (⚠). `parity_check.js` visar hoppet i egen kolumn (tröskel 0). Rådata-vägen är immun (carry-forward) — **tas bort när `TPA_KALLA='raw'` permanent**.
